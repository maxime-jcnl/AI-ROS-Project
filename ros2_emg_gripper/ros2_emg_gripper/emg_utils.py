...
-> Le filtrage et le calcul des "caractéristiques" (features) DOIVENT être
   EXACTEMENT les mêmes pendant l'entraînement du modèle ET pendant le
   fonctionnement en temps réel dans ROS2. Si on les recalculait
   différemment des deux côtés, le modèle recevrait des chiffres qu'il n'a
   jamais vus -> prédictions fausses. On centralise donc tout ici.
...
# fenêtre par 5 nombres "parlants" par canal.
